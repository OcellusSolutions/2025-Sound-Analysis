...
Expected filename format: <apiary>_<colony>_<yyyy_mm_dd>.wav
"""
...
FILENAME_RE = re.compile(
    r"^(?P<apiary>.+?)_(?P<colony>.+?)_(?P<date>\d{4}_\d{2}_\d{2})$"
)
...
def parse_filename(path: str) -> tuple[str, str, str]:
    ...
    if not match:
        raise ValueError(
            "Filename must match '<apiary>_<colony>_<yyyy_mm_dd>.wav'. "
            f"Got: {os.path.basename(path)}"
        )
    date_raw = match.group("date")
    date = date_raw.replace("_", "-")
    return match.group("apiary"), match.group("colony"), date
...
def analyze_file(
    ...
    apiary, colony, date = parse_filename(path)
    info = sf.info(path)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    default_output = os.path.join(os.path.expanduser("~"), "Desktop", "analysis.xlsx")
