...
def main() -> None:
    ...
    wav_files: list[str] = []
    for entry in args.wav_files:
        expanded = os.path.expanduser(entry)
        if os.path.isdir(expanded):
            for name in sorted(os.listdir(expanded)):
                if name.lower().endswith(".wav"):
                    wav_files.append(os.path.join(expanded, name))
        else:
            wav_files.append(expanded)
    if not wav_files:
        raise ValueError("No WAV files found in the provided paths.")

    if args.output is None:
        apiary, colony, *_ = parse_filename(wav_files[0])
        # Use a single output file per apiary/colony, regardless of mic suffix.
        base_name = f"SoundAnalysis_{apiary}_{colony}"
        desktop_dir = os.path.join(os.path.expanduser("~"), "Desktop")
        candidate_xlsx = os.path.join(desktop_dir, f"{base_name}.xlsx")
        candidate_csv = os.path.join(desktop_dir, f"{base_name}.csv")
        if os.path.exists(candidate_xlsx):
            output_path = candidate_xlsx
        elif os.path.exists(candidate_csv):
            output_path = candidate_csv
        else:
            output_path = candidate_xlsx
    else:
